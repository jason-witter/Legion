...
```
Friday → Babbage → Oppenheimer → Lovelace → Friedman → Curie → Asimov → Friday
```
...
**Asimov** (Tone Pass) runs a final scrub on the mob's output before handoff, removing AI writing tells from code comments, the PR description, and the commit message against a defined pattern catalog. Edits in place; terminal, never loops back.
