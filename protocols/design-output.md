...
2. **Write the design document** to `scratch/output/_<feature-slug>/babbage_<N>.md` where N is the next available number starting at 1 (e.g., `babbage_1.md`, `babbage_2.md`). `scratch/` resolves to the Legion repo per `rules/scratch-path-resolution.md`. See format below. This is the handoff to the implementer.
