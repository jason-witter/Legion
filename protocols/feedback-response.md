...
If a deployment-specific handoff protocol exists, follow it. Otherwise write the handoff per `protocols/local/scratch-output-conventions.md` (e.g., `scratch/output/YYYY-MM-DD/<task-slug>/handoff.md`) and report the path to the user.

After the handoff, write a PR description per `protocols/local/scratch-output-conventions.md` (e.g., `scratch/output/YYYY-MM-DD/<task-slug>/pr-description.md`). Since this protocol operates on an existing PR, the description is an update draft - it summarizes the feedback addressed in this iteration. The user decides whether to append it to the existing PR body or replace it. See the **PR Description** section in `mob-pipeline` for format guidance.
