...
If a deployment-specific handoff protocol exists, follow it. Otherwise write the handoff to `scratch/output/_<feature-slug>/handoff.md` and report the path to the user. `scratch/` resolves to the HopLegion repo per `rules/scratch-path-resolution.md`. Use `handoff_<N>.md` where N is the next available number starting at 1 (e.g., `handoff_1.md`, `handoff_2.md`).
