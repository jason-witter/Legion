...
Before writing new reports, scan `scratch/output/` across all date subdirectories for directories matching `*/pr-<number>-*/digest.md`. Remove any whose PR number is not in the current input set — those PRs are no longer open and the digests are stale. Remove empty PR directories left behind, and empty date directories after that. If no matching files exist, skip silently.

Write one report per PR per `protocols/local/scratch-output-conventions.md` (e.g., `scratch/output/YYYY-MM-DD/pr-<number>-<slug>/digest.md`). Create directories as needed. If run multiple times on the same day, overwrite existing reports for the same PR numbers.
