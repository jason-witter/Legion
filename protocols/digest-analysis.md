...
- **Resolved conversations** — inline threads marked resolved. If resolution data is provided, use it. If unavailable, treat unresolved as the default and note the data gap in the report.
...
Before writing new reports, scan `scratch/output/` across all date subdirectories for files matching `*/digests/pr-<number>-*.md`. Remove any whose PR number is not in the current input set — those PRs are no longer open and the digests are stale. Remove empty date directories left behind. If the digests directory does not exist or contains no matching files, skip silently.

Write one report per PR to `scratch/output/YYYY-MM-DD/digests/pr-<number>-<brief-slug>.md` where the date is today's date, `<number>` is the PR number, and `<brief-slug>` is a 3-5 word kebab-case summary derived from the PR title. Create the date directory if it does not exist. If run multiple times on the same day, overwrite existing reports for the same PR numbers.
