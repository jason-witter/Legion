...
# Optional, for context beyond the diff: fetch file at a specific ref
...
3. **Fetch existing review comments** — Reviews, inline comments, and thread resolution are fetched via `protocols/pr-data-fetch.md`. Use the thread resolution data to annotate each inline comment: join on `first_comment_id` (GraphQL) matching `id` (REST). Comments in a resolved thread are excluded from findings — they belong in the "Already Resolved" section of the report (or omitted entirely if the section would add no signal). Unresolved threads are treated normally. Read what other reviewers (human and bot) have already said. Note any unresolved threads, requested changes that may or may not have been addressed, and any context that affects what this review should focus on. Do not repeat findings already raised and acknowledged — add only what is new or unresolved.
