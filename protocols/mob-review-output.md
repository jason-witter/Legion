...
- `feature-slug` — kebab-case identifier for the feature (e.g., `user-settings-backfill`). Once a PR number is known, prefix it: `pr-1234-user-settings-backfill`. Caller-supplied or derived from the task description.

## Steps

1. **Determine the feature directory** — `scratch/output/_<feature-slug>/`. `scratch/` resolves to the framework repo per `rules/scratch-path-resolution.md`. If the directory does not exist, create it before writing.
...
- Resolve the framework repo path from the additionalDirectory registration or context before writing. Use absolute paths for all file operations.
