...
1. **Determine the filename** — For a single-agent cycle, use `iris-<agent-slug>.md` (e.g., `iris-grace.md`). For a multi-agent comparison, use a combined slug listing all agents reviewed (e.g., `iris-kelly-vera-overlap.md`).

2. **Write the report** to `scratch/output/<date>/agent-reviews/<filename>`. See format below.
