...
The orchestrator maintains a single accumulating narrative document as a sequential pipeline progresses through its stations. The narrative is a record for the user's audit trail. It is not used for inter-station communication; agents read prior artifacts from the feature directory, not from the narrative.

## Usage

Invoke this protocol when running a sequential pipeline where each station hands off to the next. After each station completes, the orchestrator distills that station's output into the narrative.
