...
After Curie passes validation and before writing the handoff document, Friday squashes the pipeline's working commits into a single commit on the target branch. The squash commit message reflects what the code does, with no agent names, no pipeline references, and no revision history. Transport of that squashed commit (worktree removal, branch positioning) follows the worktree rule.
...
Write the handoff to `scratch/output/_<feature-slug>/handoff.md`. `scratch/` resolves to the framework repo per `rules/scratch-path-resolution.md`. Use `handoff_<N>.md` where N is the next available number starting at 1 (e.g., `handoff_1.md`, `handoff_2.md`). Report the path to the user.
