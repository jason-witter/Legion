...
### 8. Self-Orientation from the Feature Directory

Pipeline agents receive a feature slug and work location from the orchestrator. The definition must tell the agent how to orient: what artifacts to look for in the feature directory (`scratch/output/_<feature-slug>/`), what to read in the codebase, and in what order.

This is a deliberate architectural choice. The orchestrator does not pass prior artifacts, summaries, or attention directives. Agents that receive pre-digested context anchor to the orchestrator's interpretation rather than forming independent judgments. Self-orientation from the feature directory prevents this anchoring. See `VISION.md` ("Why Feature Directory Communication") for the full rationale.

```
# Good — definition tells the agent where to look
Lovelace receives a feature slug and work location. She finds the design
document and any critique reports in the feature directory, reads the
relevant codebase, and implements the design.

# Bad — definition assumes orchestrator will provide context
Lovelace receives a design document and implements it as production-ready code.
```

The key difference: the first version makes orientation the agent's responsibility. The second assumes someone else will hand the agent what it needs.
