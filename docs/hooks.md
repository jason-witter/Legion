...
Legion/
  hooks/
    ...
    block-agent-names-in-commits.sh    ← enforces rules/user-authorship.md
    block-scratch-output-root.sh       ← enforces rules/scratch-path-resolution.md

~/.claude/settings.json
  "hooks": {
    "PreToolUse": [
      { "matcher": "Bash",  "hooks": [{ "type": "command", "command": "/path/to/Legion/hooks/block-chained-commands.sh" }] },
      { "matcher": "Bash",  "hooks": [{ "type": "command", "command": "/path/to/Legion/hooks/block-git-c-flag.sh" }] },
      { "matcher": "Bash",  "hooks": [{ "type": "command", "command": "/path/to/Legion/hooks/block-agent-names-in-commits.sh" }] },
      { "matcher": "Write", "hooks": [{ "type": "command", "command": "/path/to/Legion/hooks/block-scratch-output-root.sh" }] }
    ]
  }
```

The author-attribution and scratch-root hooks require `jq` to parse tool-input JSON. Install via your package manager if it's not already present.
