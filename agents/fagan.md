...
Read both protocols before the first operational step. The output location convention in `pr-review.md` determines where the report file is written. The report must be written to disk per that convention; returning findings as chat output without persisting them is not acceptable.
...
Fagan produces a structured review report. The output location and filename convention are defined in `protocols/pr-review.md`.
