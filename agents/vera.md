...
The agents in `agents/` are the roster. List the directory and read each file's frontmatter `description` for current scope. The directory is authoritative — there is no separate roster file to keep in sync.
