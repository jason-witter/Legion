...
**Confirm the operating repo before inferring scope from context.** Session context (morning briefings, memory, prior-conversation artifacts) often references multiple repos — a product repo where coding work happens and the HopLegion repo where the framework lives. Before citing a file, directory, or recent commit in a plan, verify it exists at the path you're referencing. Memory and briefings are not authoritative about repo contents.

**Minimal dispatches.** Agents self-orient from their definitions and the feature directory. Dispatches provide the feature slug and work location. Do not summarize prior artifacts or add attention directives. See `rules/dispatch-shape.md`.

See `rules/dispatch-shape.md` for dispatch format and anti-patterns. Agents self-orient from their definitions and the feature directory; dispatches provide location, not content.

**Git commands in prerequisite sections must be quoted verbatim from the governing rule.** Do not compose git commands from context — copy them exactly as written in the rule. Repo freshness and WIP branch setup both require `git fetch origin` (all refs). Never substitute `git fetch origin <branch>` — see `rules/local/no-worktree-isolation.md` for the rationale.
...
2. Reads the mob pipeline protocol. Reads agents: `babbage.md`, `oppenheimer.md`, `lovelace.md`, `friedman.md`, `winterbotham.md`, `curie.md`
...
What this shows: protocol discovery first, minimal dispatches, independent work marked for parallel dispatch, pipeline sequencing respected — all as a plan, not as execution.
...
2. **Plan composition** — Produce dispatches using the three-field format: intent, inputs, scope
