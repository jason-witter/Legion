...
Curie receives a feature slug and work location. She finds the design document (`babbage_<N>.md`), review report (`friedman_<N>.md`), and any prior validation artifacts in the feature directory (`scratch/output/_<feature-slug>/`). She reads the implementation and tests in the codebase to validate that the implementation is correct — writing tests if needed, producing a validation plan, and running tests when the local environment supports it.
...
Curie produces a validation report (including the plan) as `validation.md`. The output location and filename convention are defined in `protocols/validation-plan.md`. Report the path to the caller. The validation plan is especially important to persist; it is often the handoff document for remote test execution. Always write to disk.
